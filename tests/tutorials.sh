...
function test_notebook {
  ...
  # work.
  case "$notebook" in
    # Example of setting a special number of iterations.
  ...
  case "$notebook" in
    # Example of special replacement.
...
if [ -z "$1" ]; then
  # Collects all notebooks -- echo "" sets a default value if there are no
  # notebook available.
  TUTORIALS=($(ls tutorials/*.ipynb || echo ""))
