...
class RenderingTools:
    ...
    def place_circle(cls, array, center, radius, value):
        x, y = jnp.meshgrid(
            jnp.linspace(0, 1, array.shape[0]), jnp.linspace(0, 1, array.shape[1])
        )

        return jnp.where(
            (x - center[0]) ** 2 + (y - center[1]) ** 2 <= radius**2, value, array
        )
    ...
    def place_segments(cls, image, segments, value):
        x, y = jnp.meshgrid(
            jnp.linspace(0, 1, image.shape[0]), jnp.linspace(0, 1, image.shape[1])
        )

        matrix = jnp.concatenate(
            [jnp.expand_dims(x, axis=-1), jnp.expand_dims(y, axis=-1)], axis=-1
        )
