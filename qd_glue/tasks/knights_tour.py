...
class KnightsTour(QDTask):
    ...
    def __init__(self, method):
        ...
        max_bound = 8
        self._measure_space_dims = [(0, 8),
                                    (0, 8)]
        
        self.directions = np.array([[1, 2], [2, 1], [-1, 2], [-2, 1], [1, -2], [2, -1], [-1, -2], [-2, -1]])
        self.grid_w = 8
        self.grid_h = 8
        ...
        if method == "vae":
            ...
            self.fitness_model.load_state_dict(torch.load("qd_glue/tasks/vae_model.pt", map_location=torch.device(device)))
    ...
    def calculate(self, g):
        ...
        moves = np.concatenate((np.array([[g[0], g[1]]]), self.directions[g[2:]]), axis=0)
        ...
        for i in visited:
            ...
            if coord in valid_visited:
                break #already seen
            elif coord[0] >= self.grid_w or coord[0] < 0 or coord[1] >= self.grid_h or coord[1] < 0:
                break #out of bounds
            else:
    ...
    def evaluate(self, parameters, random_key=None):
        ...
        if (self._method == "hand"):
            ...
            measures_batch = self.fitness_model.forward_features(torch.Tensor(parameters))
            objective_batch = np.array([self.calculate(parameters[g])[2] for g in range(parameters.shape[0])])
